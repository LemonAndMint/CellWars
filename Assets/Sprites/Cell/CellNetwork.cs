...
{
    public class CellNetwork : MonoBehaviour
    {
        ...
        public void Add(int currCellID, CellStats cellToBeBoundStat, float boundLength){

            CellNode? targetCellNode = _searchRecursive(MainCellNode, currCellID);

            if(targetCellNode != null){

                targetCellNode.Add(cellToBeBoundStat, boundLength);

            }

        }

        private CellNode _searchRecursive(CellNode currNode, int targetCellID){
    ...
    public class CellNode{
        ...
        public CellNode(CellStats cellStats){

            Stats = cellStats;

        }

        public void Add(CellStats cellToBeBoundStat, float boundLength){

            int unOccipiedIndex = GetNearestEmptyIndex();

            if(unOccipiedIndex != -1){

                CellNode boundedCell = new CellNode(cellToBeBoundStat);
